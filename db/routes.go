...
func InsertRoute(route structs.Route) error {
	_, err := Pool.Exec(context.Background(), `
		INSERT INTO routes (id, from_system, to_system, price_per_m3)
...
func UpdateRoute(route structs.Route) error {
	_, err := Pool.Exec(context.Background(), `
		...
		SET from_system = $1,
		    ...
		    price_per_m3 = $3
		WHERE id = $4
	`, route.From, route.To, route.PricePerM3, route.ID)
...
func GetAllRoutes() ([]structs.Route, error) {
	rows, err := Pool.Query(context.Background(), `
		SELECT id, from_system, to_system, price_per_m3
		FROM routes
	`)
	if err != nil {
		return nil, err
	...
	for rows.Next() {
		var r structs.Route
		if err := rows.Scan(&r.ID, &r.From, &r.To, &r.PricePerM3); err != nil {
			return nil, err
		}
